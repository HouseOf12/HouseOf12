...
const News = () => {
  ...
  const retro = () => {
    axios
      .get(`https://mercuryretrogradeapi.com?date=${year}-${month}-${date}`, {})
      .then((response) => {
    ...
    <ChakraProvider theme={theme}>
      ...
      <Box marginTop="50px">
        ...
        {retrograde.is_retrograde === true ? (
          <Box>
            <HStack justifyContent="center">
              <Box>
                <Image
                  height="22vh"
                  src="https://media.giphy.com/media/lIeUAWuLK7cv6/giphy.gif"
                />
              </Box>
              <Box
                as="p"
                ...
                fontSize="2vh"
              >
                "Yes, that may account for the weirdness... Mercury is known as
                the planet that rules all communication, B.C.-based astrologist
                Georgia Nicols explains. This means that when Mercury appears
                retrograde, people may experience communication issues, problems
                with technology, or even things like lost mail Be careful not to
                make huge life changing decisions during this time, Mercury in
                retrograde has a way of making sure it won’t work out as
                planned. This goes for things like changing jobs, a move,
                plastic surgery, signing contracts, etc. Be cautious of new job
                opportunities or business offers, they can easily turn out to be
                nothing like what you had first expected. Take this time to
                relax, heal old wounds, and protect your energy."
              </Box>
            </HStack>
            <Box
              height="1.5px"
              bgColor="rgba(212, 175, 53, 0.5)"
              width="50%"
          ...
          <Box>Mercury is not in Retrograde right now.</Box>
        )}
      </Box>
    </ChakraProvider>
  );
